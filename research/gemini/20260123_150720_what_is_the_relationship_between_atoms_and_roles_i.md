...
def analyze_atoms_and_roles(file_path="particle/docs/MODEL.md"):
    """
    ...
    # Enumerate the atoms
    def get_atoms(file_path="particle/schema/fixed/atoms.json"):
        try:
             with open(file_path, "r") as f:
                atoms = json.load(f)
            ...
            return 0, "particle/schema/fixed/atoms.json"

    num_atoms, atoms_file = get_atoms()

    # Enumerate the roles
    def get_roles(file_path="particle/schema/fixed/roles.json"):
        try:
            with open(file_path, "r") as f:
                roles = json.load(f)
            ...
            return 0, "particle/schema/fixed/roles.json"
    ...
    file_content = extract_code_from_file("particle/src/core/unified_analysis.py")
    imports = extract_dependencies(file_content)

    # ---------------------------------------------------------
    # Extract the most common atom by type
    # ---------------------------------------------------------
    def get_most_common_atom_by_type(file_path="particle/src/core/full_analysis.py"):
