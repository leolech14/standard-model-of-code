...
| :--- | :--- | :--- | :--- |
| **THE BODY** | `particle/` | **The Physics Engine.** It parses source code, identifies "Atoms," classifies them, and builds a dependency graph. It creates the "truth." | `Collider` (CLI Tool) |
| **THE BRAIN** | `wave/` | **The Intelligence Layer.** It manages AI context, runs automated audits, facilitates queries, and mirrors data to the cloud. It *uses* the truth. | `analyze.py` (AI Interface) |

---

### 2. The Body: `particle` (The Collider)
...
### 3. The Brain: `wave` (The Intelligence)
...
*   **`wave/tools/ai/analyze.py`**: The "Swiss Army Knife" for querying the codebase.
    *   `python analyze.py "Explain the architecture"` (Uses ACI)
    *   `python analyze.py --verify pipeline` (Runs HSL audit)
*   **`particle/cli.py`**: The entry point for the analysis engine.
    *   `./collider full src/core` (Runs the full analysis pipeline)
*   **`wave/config/analysis_sets.yaml`**: Defines "views" of the codebase (e.g., `pipeline`, `theory`) that ACI loads for specific query types.
*   **`wave/docs/agent_school/`**: Contains the rules of engagement (`DOD.md`, `WORKFLOWS.md`) for AI agents.
