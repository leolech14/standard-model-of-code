...
**Evidence**:
*   **Definition**: `wave/tools/ai/aci/tier_router.py:108` defines `HYBRID = "hybrid"`.
*   **Routing**: `tier_router.py:209-216` assigns `Tier.HYBRID` for mixed internal/external scope queries.
*   **Execution Gap**: `wave/tools/ai/analyze.py:1618` (approximate).
...
**Proposed Fix**:
Modify `wave/tools/ai/analyze.py` to intercept `Tier.HYBRID`. A simple implementation runs Perplexity first, then injects that result into the standard context context.
...
**Evidence**:
*   **Source**: `wave/tools/ai/aci/semantic_matcher.py`
...
*   **Config**: `wave/config/analysis_sets.yaml` contains `pipeline`, `viz_core`, `classifiers`, `repo_tool`, etc., but matches very few of the semantic suggestions.
...
```python
# wave/tools/ai/aci/semantic_matcher.py
...
**Evidence**:
*   **Config**: `wave/config/aci_config.yaml:80`
...
*   **Logic**: `wave/tools/ai/aci/query_analyzer.py:344`
    ```python
