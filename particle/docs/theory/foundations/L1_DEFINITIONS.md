...
|-------------|-------------|-----------------|
| Pipeline | `particle/src/core/` | `particle/docs/specs/` |
| Visualization | `particle/src/core/viz/` | `particle/docs/specs/UI*.md` |
| Governance | `.agent/tools/` | `.agent/registry/`, `.agent/specs/` |
| AI Tools | `wave/tools/ai/` | `wave/config/` |
| Theory | (N/A - no executable code) | `particle/docs/` |
...
| Aspect | Genetic Locus | SMC Locus | Parallel Strength |
|--------|---------------|-----------|-------------------|
| **Structure** | 1D linear (chromosome band) | n-dimensional (5+ axes) | DIVERGENT |
| **Notation** | `17q21.31` (chr:arm:region:band) | `⟨L3, R1, T1, Query, (1,2,2,1)⟩` | PARALLEL |
| **Uniqueness** | Each gene has ONE locus | Each atom has ONE locus | IDENTICAL |
| **Invariance** | Locus fixed across generations | Locus fixed across refactors | PARALLEL |
| **Container** | Chromosome → Genome | Package → Codebase → Projectome | PARALLEL |
| **Identity** | Locus defines gene identity | Locus defines atom identity | IDENTICAL |

##### Complete Vocabulary Mapping

| Genetic Term | SMC Equivalent | Definition | Relationship Quality |
|--------------|----------------|------------|---------------------|
| **Chromosome** | Codebase/Package | Container structure holding functional units | Strong |
| **Gene** | Atom | The functional unit being located and classified | Strong |
| **Locus** | LOCUS | Fixed position in abstract classification space | Identical intent |
| **Allele** | Variant/Instance | Same locus, different implementation | Strong |
| **Genome** | Projectome | Complete set of all genetic/code material | Strong |
| **Genotype** | Full Classification | Static description — what it IS (structure) | Strong |
| **Phenotype** | Runtime Behavior | Dynamic expression — what it DOES (execution) | Strong |
| **Mutation** | Code Change | Alteration at a locus; may or may not change phenotype | Moderate |
| **Linkage** | Coupling | Nearby loci tend to change/travel together | Moderate |
| **Expression** | Invocation | Gene → protein; Atom → runtime behavior | Strong |
| **Regulatory region** | Decorator/Middleware | Controls when/how expression occurs | Moderate |
| **Epistasis** | Shadowing | Gene A masks Gene B; Module A shadows Module B | Moderate |
| **Pleiotropy** | High Fan-out | One gene/function affects multiple traits/systems | Strong |
| **Linkage disequilibrium** | Tight Coupling | Nearby genes/code inherited together | Strong |

##### Exploitable Metaphors (What We CAN Use)

**1. EXPRESSION (Genotype → Phenotype)**
```
GENETICS:  Gene is transcribed → mRNA → protein → trait
SMC:       Atom is classified → invoked → runtime behavior → effect

The distinction between WHAT something IS (genotype/classification)
and WHAT it DOES (phenotype/behavior) is fundamental to both domains.
```

**2. REGULATORY REGIONS**
```
GENETICS:  Promoters, enhancers, silencers control gene expression
SMC:       Decorators, middleware, guards control function invocation

@auth_required        ← "promoter" - must be present for expression
@cache(ttl=300)       ← "enhancer" - modifies expression behavior
@deprecated           ← "silencer" - suppresses/warns against expression
```

**3. EPISTASIS (Masking Effects)**
```
GENETICS:  Gene A masks the effect of Gene B
SMC:       Module A shadows Module B (import precedence)

from utils import helper     # Gene A expressed
from lib import helper       # Gene B masked (same locus, different allele wins)
```

**4. PLEIOTROPY (Multiple Effects)**
```
GENETICS:  One gene affects multiple phenotypic traits
SMC:       One function affects multiple systems (high out-degree)

def update_user():           # Pleiotropic function
    db.save(user)            # Effect 1: persistence
    cache.invalidate(user)   # Effect 2: cache state
    notify.send(user)        # Effect 3: external system
    audit.log(user)          # Effect 4: compliance

High pleiotropy = high RESPONSIBILITY score in RPBL
```

**5. LINKAGE DISEQUILIBRIUM (Co-inheritance)**
```
GENETICS:  Nearby genes on chromosome inherited together
SMC:       Tightly coupled code changes together

When you modify parse(), you almost always modify validate().
They exhibit "linkage" — high co-change frequency.
Detectable via git history analysis.
```
...
GENETICS:                         SMC:
─────────────────────────────────────────────────────────────
Silent mutation                   Refactor (no behavior change)
  (codon change, same protein)      (rename, restructure, same output)

Missense mutation                 Bug fix / Feature change
  (different amino acid)            (different behavior)

Nonsense mutation                 Breaking change
  (premature stop codon)            (function no longer works)

Frameshift mutation               Architectural violation
  (everything downstream wrong)     (cascading breaking changes)
```

##### Where The Parallel Breaks Down

| Genetic Concept | Why No SMC Equivalent | Notes |
|-----------------|----------------------|-------|
| **Base pairs (A,T,G,C)** | Code has tokens, not 4-letter alphabet | Tokens are variable-length, context-dependent |
| **Codons → Amino acids** | No 3→1 mapping layer | AST is tree, not linear sequence |
| **Introns/Exons** | No "junk" sections spliced out | All code is potentially executable |
| **Epigenetics** | No heritable non-sequence modifications | Config/env could weakly parallel this |
| **Ploidy (diploid/haploid)** | No "copies per cell" concept | Though git branches parallel this somewhat |
| **Sexual reproduction** | Code doesn't "mate" | Though merging has recombination aspects |
| **Natural selection** | No automatic fitness-based survival | Though dead code detection is analogous |
| **Horizontal gene transfer** | Actually EXISTS in code | Copy-paste, library imports (stronger than genetics!) |

##### Key Divergences

| Aspect | Genetics | SMC | Implication |
|--------|----------|-----|-------------|
| **Dimensionality** | 1D (linear on chromosome) | 5D+ (Level, Ring, Tier, Role, RPBL) | SMC needs richer coordinate system |
| **Physical substrate** | DNA molecule (material) | None (purely Platonic/conceptual) | SMC locus is abstract, not physical |
| **Position type** | Continuous (base pairs 1-3B) | Discrete (categorical axes) | Different mathematical treatment |
| **Inheritance** | Vertical (parent→child) | Horizontal (imports, deps) | Code inheritance is network, not tree |
| **Recombination** | Meiosis, crossover | Refactoring, git merge | Both shuffle existing material |
| **Time scale** | Generations (years) | Commits (hours/days) | Code evolves faster |
| **Selection pressure** | Environment, fitness | Requirements, tech debt | Both drive evolution |

##### Novel Insights From The Parallel

**1. Locus Stability Principle**
```
GENETICS:  A gene's locus is invariant even as alleles change.
           BRCA1 is always at 17q21.31, regardless of variant.

SMC:       An atom's Locus should be invariant even as implementation changes.
           getUserById() stays at ⟨L3, R1, T1, Query, (1,2,2,1)⟩
           even if internals are completely rewritten.

INSIGHT:   If refactoring changes the Locus, it's not a refactor —
           it's a NEW entity. This solves the Ship of Theseus problem.
```

**2. Allelic Variation**
```
GENETICS:  Same locus can have different alleles (variants).
           BRCA1 has normal allele and cancer-risk allele.

SMC:       Same Locus can have different implementations.
           ⟨L3, R1, T1, Query, (1,2,2,1)⟩ could be:
             - SQL implementation
             - NoSQL implementation
             - Mock implementation

INSIGHT:   Locus defines the ROLE, not the IMPLEMENTATION.
           Polymorphism is "allelic variation" in code.
```

**3. Genetic Load → Technical Debt**
```
GENETICS:  Genetic load = accumulated deleterious mutations
           in a population, reducing fitness.

SMC:       Technical debt = accumulated suboptimal decisions
           in a codebase, reducing maintainability.

INSIGHT:   Both accumulate silently, both have carrying costs,
           both require active management to reduce.
```

**4. Founder Effect**
```
GENETICS:  Small founding population → reduced genetic diversity
           → certain alleles overrepresented.

SMC:       Early architectural decisions → reduced pattern diversity
           → certain patterns overrepresented (for better or worse).

INSIGHT:   The first 1000 lines of code have outsized influence,
           just as founding population genetics shapes descendants.
```

##### Gemini-Validated Additional Concepts (2026-02-01)

**Added from validation session:**

| Genetic Concept | SMC Mapping | Insight | Strength |
|-----------------|-------------|---------|----------|
| **Epigenetics** | CONTEXTOME | Config files change behavior without changing code (genotype). The Contextome IS the epigenetic layer. | 90% |
| **Horizontal Gene Transfer** | Package managers | npm/pip imports are literal DNA splicing from foreign organisms. Stronger in code than biology. | 92% |
| **Introns/Exons** | Comments/Code | Compiler discards comments (introns), processes only executable statements (exons). | 75% |
| **Haplotype** | Design Patterns | Singleton, Factory = recognizable allele clusters that travel together. | 80% |
| **Lethal Allele** | Build Breaker | Valid syntax (DNA) that kills the organism at runtime (startup crash). | 85% |
| **Codon Optimality** | Linter Rules | Enforcing specific syntax allele even when phenotype is identical. | 78% |
| **Linkage Disequilibrium** | Architecture Metric | Modules that SHOULD be independent but change together = architectural drift. | 88% |

**Critical Correction: Directed Mutation**

The weakest parallel is "mutation" (40% strength) because:
- **Biology:** Mutation is RANDOM (stochastic errors)
- **Code:** Changes are INTENTIONAL (teleological, purpose-driven)

Resolution: Code changes should be called "Directed Mutation" (like CRISPR editing), not random mutation. This aligns with the Crystallization concept in CODESPACE_ALGEBRA.md.

##### Parallel Applicability Score (CDPS Methodology)

**Methodology:** See [CROSS_DOMAIN_METHODOLOGY.md](./CROSS_DOMAIN_METHODOLOGY.md) for formal calculation.

**Formula:** `CDPS = (0.20×D1) + (0.25×D2) + (0.30×D3) + (0.15×D4) + (0.10×D5) × 10`

| Concept | D1 | D2 | D3 | D4 | D5 | CDPS | Classification |
|---------|----|----|----|----|----|----|----------------|
...
- D5: Scale Invariance (Fractal) - 10%

**Overall Assessment:** The genetic parallel achieves **Strong Parallel** status for core concepts (genotype, epigenetics, HGT), **Homology** for secondary concepts (pleiotropy, linkage), and **Metaphor** only for mutation (due to teleology mismatch in D2).

##### Recommended Usage

**USE the genetic metaphor when:**
- Explaining Locus to newcomers (intuitive)
- Discussing identity vs implementation (genotype/phenotype)
- Analyzing code evolution patterns (mutation, selection)
- Understanding coupling (linkage)

**AVOID the genetic metaphor when:**
- Discussing dimensionality (genetics is 1D, SMC is nD)
- Explaining horizontal dependencies (genetics is vertical)
- Physical vs conceptual substrate discussions
