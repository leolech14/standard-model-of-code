...
window.initializeModules = function (data) {
    ...
    // Expose payload globally for modules that need it (e.g. encoding-view reads view_registry)
    window.COLLIDER_DATA = data;
    ...
    // Initialize Encoding View (needs COLLIDER_DATA set above for view_registry)
    if (typeof ENCODING_VIEW_MODULE !== 'undefined' && ENCODING_VIEW_MODULE.init) {
        ENCODING_VIEW_MODULE.init();
        console.log('[MAIN] ENCODING_VIEW module initialized');
    }

    // Initialize Syndrome Dashboard (needs COLLIDER_DATA for chemistry.syndromes)
    if (typeof SYNDROMES !== 'undefined' && SYNDROMES.init) {
        SYNDROMES.init();
        console.log('[MAIN] SYNDROMES module initialized');
    }

    // Initialize Temporal Overlay (needs COLLIDER_DATA for temporal_analysis)
    if (typeof TEMPORAL !== 'undefined' && TEMPORAL.init) {
        TEMPORAL.init();
