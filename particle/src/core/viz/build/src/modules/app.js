// Process/global polyfills handled by globals.js
// CDN globals (THREE, pako, ForceGraph3D) loaded via <script> tags
const ConvexGeometry = window.THREE?.ConvexGeometry;

// Read payload from <script id="collider-payload"> tag instead of inline string
const _payloadEl = document.getElementById('collider-payload');
const COMPRESSED_PAYLOAD = _payloadEl ? _payloadEl.textContent.trim() : '';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Initialize Holarchy tab UI (view toggle between force-graph and holarchy)
...
// FILE_GRAPH, FILE_NODE_IDS, EXPANDED_FILES, FILE_EXPAND_MODE - local vars (window getters in file-viz.js are backward compat only)
let FILE_GRAPH = null;
let FILE_EXPAND_MODE = 'inline';
...
// COLOR ORCHESTRATOR: Using modular color-engine.js implementation
// SINGLE SOURCE OF TRUTH for ALL colors in the visualization.
const Color = window.Color;
const ColorOrchestrator = window.Color;
    ...
    try {
        Graph = ForceGraph3D({
            ...
            .onEngineStop(() => {
                ...
                // Center orbit target on data centroid
                if (window._updateOrbitTarget) window._updateOrbitTarget();
            });
    } catch (err) {
    ...
    if (controls) {
        ...
        window._updateOrbitTarget = function() {
            ...
            for (const n of nodes) {
                if (n.x !== undefined) { cx += n.x; cy += n.y; cz += (n.z || 0); count++; }
            }
            if (count > 0) {
                ctrl.target.set(cx / count, cy / count, cz / count);
                ctrl.update();
            }
        };

        // Update periodically during warmup so rotation works immediately
        let _orbitUpdates = 0;
        const _orbitInterval = setInterval(() => {
            window._updateOrbitTarget();
            if (++_orbitUpdates >= 10) clearInterval(_orbitInterval);
        }, 500);
    }
