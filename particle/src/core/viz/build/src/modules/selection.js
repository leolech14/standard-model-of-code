...
function updateSelectionPanel() {
    ...
    // Clear intel state on every update
    if (typeof NODE_INTEL !== 'undefined' && NODE_INTEL.clearIntelState) {
        NODE_INTEL.clearIntelState();
    }

    if (nodes.length === 1 && typeof NODE_INTEL !== 'undefined' && NODE_INTEL.renderNodeCard) {
        // Single node: render rich intelligence card
        NODE_INTEL.renderNodeCard(nodes[0], body);
...
window.getSelectedNodes = function() { return SELECT.getSelectedNodes(); };
window.setSelection = function(ids, additive) { SELECT.set(ids, additive); };
window.toggleSelection = function(node) { SELECT.toggle(node); };
window.clearSelection = function() { SELECT.clear(); };
window.maybeClearSelection = function() { SELECT.maybeClear(); };
window.startSelectionAnimation = function() { SELECT.startAnimation(); };
window.stopSelectionAnimation = function() { SELECT.stopAnimation(); };
window.ensureNodeOverlays = function(node) { return SELECT.ensureOverlays(node); };
window.updateOverlayScale = function(node) { SELECT.updateOverlayScale(node); };

// UI functions (moved from app.js)
window.updateSelectionPanel = function() { SELECT.updateSelectionPanel(); };
window.showSelectionModal = function() { SELECT.showSelectionModal(); };
window.hideSelectionModal = function() { SELECT.hideSelectionModal(); };
window.initSelectionModal = function() { SELECT.initSelectionModal(); };
window.updateSelectionBox = function(rect) { SELECT.updateSelectionBox(rect); };
window.getNodeScreenPosition = function(node) { return SELECT.getNodeScreenPosition(node); };
window.selectNodesInBox = function(rect, additive) { SELECT.selectNodesInBox(rect, additive); };
window.setupSelectionInteractions = function() { SELECT.setupSelectionInteractions(); };
window.syncSelectionAfterGraphUpdate = function() { SELECT.syncSelectionAfterGraphUpdate(); };
window.initSelectionState = function(data) { SELECT.initSelectionState(data); };
