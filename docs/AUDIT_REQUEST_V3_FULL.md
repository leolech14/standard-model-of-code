...
class SemanticIDGenerator:
    ...
    def from_particle(self, particle: Dict, smells: Dict[str, float] = None) -> SemanticID:
        ...
        module_path = file_path.replace("/", ".").replace(".py", "")
        if "standard-model-of-code" in module_path:
             # Clean up path slightly for internal repos
             try:
                 idx = module_path.index("standard-model-of-code")
                 module_path = module_path[idx+len("standard-model-of-code")+1:]
             except: pass
...
```markdown
# Standard Model of Code (Spectrometer)
