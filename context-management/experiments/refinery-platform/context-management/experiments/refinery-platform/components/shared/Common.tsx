 ...
 * Ported from reference design - glassmorphic dark theme
 */
...
export const UiRow: React.FC<React.HTMLAttributes<HTMLDivElement> & { selected?: boolean }> = ({
  ...
  return (
    ...
    >
      {/* Left tick indicator */}
      <div className={`
        absolute left-0 top-1/2 -translate-y-1/2 w-0.5 h-4 bg-neutral-600 rounded-r-sm
...
export const Badge: React.FC<{
  status: 'success' | 'failed' | 'running' | 'idle' | 'warning' | 'info' | 'live' | 'critical' | string
}> = ({ status }) => {
  let color = 'bg-neutral-700';

  if (status === 'success' || status === 'live') {
    color = 'bg-emerald-500/80 shadow-[0_0_8px_-2px_rgba(16,185,129,0.5)]';
  }
  if (status === 'failed' || status === 'critical') {
    color = 'bg-rose-500/80 shadow-[0_0_8px_-2px_rgba(244,63,94,0.5)]';
  }
  if (status === 'running') {
...
export const EmptyState: React.FC<{
  ...
  <div className="flex flex-col items-center justify-center py-24 text-center opacity-60">
    ...
    {submessage && (
      <p className="text-xs text-neutral-600 mt-1 max-w-xs">{submessage}</p>
    )}
  </div>
);
