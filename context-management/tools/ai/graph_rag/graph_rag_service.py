...
class GraphRAGService:
    ...
    def _classify_query(self, question: str) -> str:
        ...
        if any(kw in q_lower for kw in ['validate', 'ground', 'theory', 'paper']):
            return "validation"
        elif any(kw in q_lower for kw in ['implement', 'code', 'function']):
            return "implementation"
        elif any(kw in q_lower for kw in ['research', 'study', 'paper']):
            return "research"
        else:
            return "general"
    ...
    def _retrieve_context(self, question: str) -> str:
        ...
        # Note: APOC might not be available. Fallback to simple pattern:
        fallback_cypher = """
        MATCH (start)-[r]-(neighbor)
        WHERE (start:Concept OR start:Paper)
          AND (toLower(start.name) CONTAINS toLower($keyword) 
               OR toLower(start.title) CONTAINS toLower($keyword))
