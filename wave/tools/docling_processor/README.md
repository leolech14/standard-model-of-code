...
# Process single file
python -m context_management.tools.docling_processor process \
    --file wave/library/references/pdf/FRISTON_2019_FreeEnergyPrincipleParticularPhysics.pdf
...
Edit `wave/config/docling_config.yaml`:

```yaml
input_dir: "wave/library/references/pdf"
output_dir: "wave/library/references/docling_output"
enable_ocr: true
...
# 3. Check output
cat wave/library/references/docling_output/latest/manifest.json | jq '.summary'
