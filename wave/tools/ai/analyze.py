...
# Ensure we can find packages in wave/services/
sys.path.append(str(PROJECT_ROOT / "wave" / "services"))
sys.path.append(str(PROJECT_ROOT / "wave" / "tools" / "ai"))
...
# =============================================================================
# See: wave/docs/specs/ANALYZE_OUTPUT_CONTRACT.md
...
try:
    ...
    try:
        from wave.services.graph_rag import GraphRAGService
    except ImportError:
...
GEMINI_RESEARCH_PATH = PROJECT_ROOT / "particle/docs/research/gemini"
...
def _save_session_log(model: str = None, total_tokens_in: int = 0, total_tokens_out: int = 0):
    ...
    save_dir = PROJECT_ROOT / "particle" / "docs" / "research" / "gemini" / "sessions"
...
def verify_domain(domain, store_name=None, output=None, index=False, candidate=None):
    ...
    if index:
        ...
        target_dir = None
        if domain == 'pipeline':
            target_dir = PROJECT_ROOT / "particle/src/core"
        elif domain == 'theory':
            target_dir = PROJECT_ROOT / "particle/docs/theory"
...
def verify_domain(domain, store_name=None, output=None, index=False, candidate=None):
    ...
    intelligence_dir = PROJECT_ROOT / "wave/intelligence"
    ...
    final_output = output or f"wave/reports/socratic_audit_latest.md"
