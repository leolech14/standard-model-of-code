...
The `particle/` subproject has solid foundational configuration but lacks modern Python packaging standards and developer experience tooling.

### 1.2 Present Configuration Files

| File | Location | Quality | Notes |
|------|----------|---------|-------|
| `setup.py` | `/particle/setup.py` | 6/10 | Version hardcoded, minimal metadata |
| `requirements.txt` | `/particle/requirements.txt` | 7/10 | Loose pinning (`>=`), no lock file |
| `.gitignore` | Root + subproject | 9/10 | Comprehensive patterns |
| `pytest.ini` | `/particle/pytest.ini` | 8/10 | Clear test discovery, missing coverage |
| `.pre-commit-config.yaml` | `/particle/.pre-commit-config.yaml` | 7/10 | Docstrings enforced, missing formatters |
| `.github/workflows/ci.yml` | `/particle/.github/workflows/ci.yml` | 7/10 | Multi-stage but incomplete |
| `.import-linter.ini` | `/particle/.import-linter.ini` | 4/10 | Wrong package name referenced |
...
**Current:** `/particle/.import-linter.ini`
```ini
[importlinter]
...
|-----------|--------|----------|
| Test files | 15 files, 120 functions | `/particle/tests/` |
| Fixtures | 10+ toy projects | `/particle/tests/fixtures/` |
| Configuration | Present | `/particle/pytest.ini` |
...
**Evidence from `/particle/.github/workflows/ci.yml`:**
...
**Problem:** `particle/README.md` references `docs/` but this directory does not exist.

**Actual documentation locations:**
```
/particle/.archive/docs_archive/    ← Archived (hidden)
/wave/docs/                          ← Active but separate
/wave/docs/theory/THEORY.md          ← Main theory doc
```
...
|-----------|------------------|--------|
| `/wave/docs/` | YES (README.md) | Good |
| `/wave/docs/theory/` | NO | Navigation gap |
| `/wave/docs/prompts/` | NO | 22 subdirs, no guide |
| `/wave/docs/operations/` | NO | Navigation gap |
| `/particle/schema/` | YES (README.md) | Good |
| `/particle/src/core/` | NO | 67 files, no guide |
| `/particle/tests/` | NO | No test guide |
...
| Researcher | wave/docs/theory/ | GOOD (when found) |
...
mkdir -p particle/docs

# Option A: Symlinks (preferred for maintenance)
ln -s ../wave/docs/theory/THEORY.md particle/docs/THEORY.md
ln -s .archive/docs_archive/legacy_theory/ATOMS_REFERENCE.md particle/docs/ATOMS_REFERENCE.md

# Option B: Move files back from archive
mv .archive/docs_archive/legacy_theory/*.md particle/docs/
```
...
**Template for `/particle/src/core/INDEX.md`:**
```markdown
# Core Module Index
...
3. Install dependencies: `cd particle && make install`
...
| File | Location |
|------|----------|
| `pyproject.toml` | `/particle/pyproject.toml` |
| `.coveragerc` | `/particle/.coveragerc` |
| `Dockerfile` | `/particle/Dockerfile` |
| `Makefile` | `/particle/Makefile` |
| `.env.example` | `/particle/.env.example` |
| `tox.ini` | `/particle/tox.ini` |
| `.editorconfig` | `/particle/.editorconfig` |
| `CHANGELOG.md` | `/particle/CHANGELOG.md` |
| `CODEOWNERS` | `/particle/CODEOWNERS` |
| `CONTRIBUTING.md` | `/particle/CONTRIBUTING.md` |
| `docs/` directory | `/particle/docs/` |
| `INDEX.md` | `/particle/src/core/INDEX.md` |
| Bug report template | `/particle/.github/ISSUE_TEMPLATE/bug_report.md` |
| Feature request template | `/particle/.github/ISSUE_TEMPLATE/feature_request.md` |
| PR template | `/particle/.github/PULL_REQUEST_TEMPLATE.md` |
| dependabot.yml | `/particle/.github/dependabot.yml` |

### 6.2 Files to Modify

| File | Change |
|------|--------|
| `/particle/README.md` | Fix 7 dead links |
| `/particle/.github/workflows/ci.yml` | Add pytest, coverage, mypy, security steps |
| `/particle/.import-linter.ini` | Fix package name from `spectrometer_v12_minimal` to `collider` |
| `/particle/.pre-commit-config.yaml` | Add black, isort hooks |
| `/particle/setup.py` | Read version from pyproject.toml |
...
```
Repository Size:           2.2 GB (particle) + 54 MB (wave)
