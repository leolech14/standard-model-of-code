...
### Lint
```bash
cd particle && ruff check src/
```

### Format
```bash
cd particle && black src/ --check
```

### Build
```bash
cd particle && pip install -e .
```
...
├── particle/  # Subproject with own CLAUDE.md
...
| `particle/CLAUDE.md` | Subproject-specific rules |
