...
**Tool**: `wave/tools/ai/analyze.py --mode forensic`
**Best For**: Verification, Auditing, "Prove it".
**Mechanism**: Gemini 2.0 Flash with strict citation instructions.
**Usage**:
```bash
python wave/tools/ai/analyze.py "List all hardcoded timeout values in the archive tool" --dir wave/tools/archive --mode forensic
```
**Why it works**: The system prompt forces the AI to output:
> `[wave/tools/archive/archive.py:L45-L48]` "timeout=30"

### 3. The Architect (Global Reasoning)
**Tool**: `wave/tools/ai/analyze.py --mode architect`
...
**Usage**:
```bash
python wave/tools/ai/analyze.py "Does the archive tool violate the 'Atom' isolation principle?" --mode architect
```
...
**Best For**: Automated semantic auditing, Antimatter Pattern detection, codebase health monitoring.
...
python wave/tools/ai/analyze.py --verify pipeline --candidate path/to/file.py
```
...
python wave/tools/ai/analyze.py --interactive --set body
```
...
python wave/tools/ai/analyze.py --index --set renderer --store-name renderer-v1

# 2. Ask questions with citations
python wave/tools/ai/analyze.py --search "How are textures loaded?" --store-name renderer-v1
```
...
python wave/tools/ai/analyze.py --verify pipeline --candidate particle/src/core/violation_stage.py
```

**Insights (Structured Output)**
```bash
python wave/tools/ai/analyze.py --mode insights --file "README.md" --output "report.json"
```
...
2.  **Consult the Architect**:
    ```bash
    .tools_venv/bin/python wave/tools/ai/analyze.py "Review tools/archive for compliance with the Standard Model" --mode architect
    ```
...
2.  **Ask for Help**:
    ```bash
    .tools_venv/bin/python wave/tools/ai/analyze.py --interactive
    ```
