...
**Status:** ACTIVE
**Location:** `wave/tools/maintenance/` (observability) and `wave/tools/ai/` (semantic)
...
**Location:** `wave/tools/ai/boundary_analyzer.py`
...
**Location:** `wave/tools/maintenance/gemini_status.py`
...
**Location:** `wave/tools/maintenance/boot.sh`
...
- repo: local
  hooks:
    - id: boundary-check
      ...
      entry: python3 wave/tools/maintenance/boundary_analyzer.py --quick --threshold 60
      language: system
  ...
  health-check:
    ...
    steps:
      ...
      - name: Boundary Analysis
        run: python3 wave/tools/maintenance/boundary_analyzer.py --json --threshold 70
```
...
| Reference | Location |
|-----------|----------|
| This doc | `wave/docs/operations/MAINTENANCE_TOOLS.md` |
| Boundary Analyzer | `wave/tools/ai/boundary_analyzer.py` |
| Gemini Status | `wave/tools/maintenance/gemini_status.py` |
| Boot Script | `wave/tools/maintenance/boot.sh` |
| Boundary Report | `.agent/intelligence/boundary_analysis.json` |
| CODOME/CONTEXTOME specs | `wave/docs/CODOME.md`, `CONTEXTOME.md` |
| DOMAINS spec | `wave/docs/DOMAINS.md` |
