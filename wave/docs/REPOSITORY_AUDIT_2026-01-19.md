...
**Status**: Pending
**Confidence**: 95%
**File**: `wave/tools/maintenance/generate_metadata_csv.py`
...
**Confidence**: 90%
**Files**:
- `particle/output/unified_real_v2/unified_analysis.json` (841 MB)
- `particle/output/unified_real_v2/collider_report.html` (345 MB)
...
|----------|-----------|----------------|
| `C1_ATOM_ENUMERATION.md` | `wave/docs/roadmaps/`, `particle/.agent/orientation/` | Keep canonical in `wave/`, symlink from `.agent/` |
| `C2_JSON_SCHEMA.md` | Same as above | Same |
| `C3_TRAINING_CORPUS.md` | Same as above | Same |
| `THEORY.md` | `wave/docs/theory/`, `particle/.agent/orientation/` | Same |
| `ROADMAP.md` | 3 locations | Keep in `particle/`, archive others |
...
| Tool | Path | Purpose |
|------|------|---------|
| `analyze_sets.py` | `wave/tools/maintenance/` | Generate set coverage metrics |
| `find_relocation_candidates.py` | `wave/tools/maintenance/` | Identify misplaced file clusters |
| `generate_metadata_csv.py` | `wave/tools/maintenance/` | Create file metadata audit |
...
| Report | Path | Content |
|--------|------|---------|
| File Metadata | `wave/output/file_metadata_audit.csv` | All 5,697 files with metadata |
| Set Coverage | `wave/output/analysis_sets_report.md` | Token counts per analysis set |
...
**Action**: Create symlinks from `particle/.agent/orientation/` → `wave/docs/`
...
**Decision**: **RELOCATE to `wave/experiments/`**

| Option | ✓ | Justification |
|--------|---|---------------|
| Keep at root | ✗ | Clutter |
| Archive | ✗ | Not legacy |
| **wave/experiments/** | ✓ | LLM experiments are Brain function, not Body physics. |

**Action**: `mv llm-threads/ wave/experiments/llm-threads/`
...
**Action**: `mv particle/output/unified_real_v2/ archive/large_outputs/`
