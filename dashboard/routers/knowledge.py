...
def get_references_status():
    """
    Get reference library status.

    Returns analysis progress, indexes, and availability.
    """
    refs_dir = PROJECT_ROOT / "context-management" / "archive" / "references"
    metadata_dir = refs_dir / "metadata"
    index_dir = refs_dir / "index"

    if not refs_dir.exists():
        return {"error": "Reference library not found"}

    try:
        # Count analyzed vs stubs
        total = 0
        analyzed = 0
        stubs = 0
        ...
        for meta_file in metadata_dir.glob("*.json"):
            ...
            try:
                ...
                if summary and summary != "[TO BE GENERATED BY LLM]" and len(summary) > 100:
                    analyzed += 1
                else:
                    stubs += 1
            except:
        ...
        # Check indexes
        indexes = {
            "catalog": (index_dir / "catalog.json").exists(),
            "concepts": (index_dir / "concept_index.json").exists(),
            "fulltext": (index_dir / "search_index.json").exists()
        }
        ...
        if indexes["concepts"]:
            try:
                concepts = json.loads((index_dir / "concept_index.json").read_text())
        ...
        if indexes["fulltext"]:
            try:
                search_idx = json.loads((index_dir / "search_index.json").read_text())
                term_count = len(search_idx)
            except:
                pass
        ...
        return {
            ...
            "analyzed": analyzed,
            "pending": stubs,
            "completion_pct": round(100 * analyzed / max(total, 1), 1),
            "indexes": indexes,
