#!/usr/bin/env python3
"""
SPECTROMETER v6 FINAL - 100% FUNCIONAL
12 Quarks + 96 Hadrons + Multi-linguagem
"""
...
# ===============================================
QUARKS = {
    'EXECUTABLES': {'color': '#FF6600', 'patterns': ['main', 'init', 'start', 'run', 'program']},
    'FILES': {'color': '#8888FF', 'patterns': ['import', 'require', 'include', 'use', 'from']},
    'MODULES': {'color': '#FFFF00', 'patterns': ['module', 'namespace', 'package', 'export']},
    'AGGREGATES': {'color': '#44FF44', 'patterns': ['class', 'struct', 'interface', 'type', 'enum']},
    'FUNCTIONS': {'color': '#8844FF', 'patterns': ['function', 'def', 'fn', 'method', 'arrow', 'lambda']},
    'CONTROL': {'color': '#FF0088', 'patterns': ['if', 'else', 'for', 'while', 'switch', 'case', 'try']},
    'STATEMENTS': {'color': '#FF8800', 'patterns': ['return', 'break', 'continue', 'throw', 'assert']},
    'EXPRESSIONS': {'color': '#FF4444', 'patterns': ['call', 'new', 'await', 'async', 'promise']},
    'VARIABLES': {'color': '#FF00FF', 'patterns': ['var', 'let', 'const', 'variable', 'assign']},
    'PRIMITIVES': {'color': '#00FF88', 'patterns': ['string', 'number', 'int', 'float', 'bool']},
    'BYTES': {'color': '#0088FF', 'patterns': ['bytes', 'buffer', 'bytearray', 'arraybuffer']},
    'BITS': {'color': '#00FFFF', 'patterns': ['bitwise', 'xor', 'and', 'or', 'shift']}
}

# ===============================================
# 96 HÁDRONS - REGRAS DE DETECÇÃO
# ===============================================
HADRON_RULES = [
    # Arquitetura de API
    ('APIHandler', ['FUNCTIONS'], ['@route', '@get', '@post', 'app.get', 'router.get', 'endpoint']),
    ('CommandHandler', ['FUNCTIONS'], ['save', 'create', 'update', 'delete', 'insert', 'remove', 'post']),
    ('QueryHandler', ['FUNCTIONS'], ['get', 'find', 'fetch', 'list', 'query', 'select', 'search']),
    ('EventHandler', ['FUNCTIONS'], ['@event', 'addEventListener', 'subscribe', 'emit', 'publish']),
    ('Middleware', ['FUNCTIONS'], ['@middleware', 'middleware', 'interceptor', 'filter', 'use']),

    # Dados
    ('Entity', ['AGGREGATES'], ['@entity', '@table', 'extends Model', 'entity', 'model', 'aggregate']),
    ('ValueObject', ['AGGREGATES'], ['@value', 'immutable', 'readonly', 'final class', 'valueobject']),
    ...
    # Testes
    ('TestFunction', ['FUNCTIONS'], ['test_', 'it(', 'should', 'expect', 'assert', '@test']),
    ('TestFixture', ['FUNCTIONS'], ['@fixture', '@before', '@after', 'setup', 'teardown']),
    ('TestCase', ['AGGREGATES'], ['testcase', 'testclass', 'describe', 'context']),
    ('Mock', ['AGGREGATES'], ['@mock', 'mock', 'stub', 'fake', 'spy']),

    # Infraestrutura
    ('DIContainer', ['AGGREGATES'], ['@inject', '@autowired', 'injector', 'container', 'provider']),
    ('Config', ['AGGREGATES'], ['config', 'settings', 'properties', 'env', 'environment']),
    ('Validator', ['FUNCTIONS'], ['validate', 'validation', 'check', 'verify', 'isvalid']),
    ('Mapper', ['FUNCTIONS'], ['map', 'mapper', 'convert', 'transform', 'todto', 'toentity']),

    # Padrões Comportamentais
    ('Observer', ['AGGREGATES'], ['observer', 'observable', 'listener', '@observer', 'subject']),
    ('Strategy', ['AGGREGATES'], ['strategy', 'algorithm', '@strategy', 'strategypattern']),
    ('Command', ['AGGREGATES'], ['command', 'execute', '@command', 'commandpattern']),
    ('State', ['AGGREGATES'], ['state', 'context', '@state', 'statepattern']),
    ('TemplateMethod', ['FUNCTIONS'], ['template', 'abstract', 'override', '@template']),
    ...
    ('Builder', ['AGGREGATES'], ['builder', 'fluent', 'with', 'builderpattern']),
    ...
    # Elementos de Código
    ('Constructor', ['FUNCTIONS'], ['__init__', 'constructor', 'new', 'create']),
    ('Getter', ['FUNCTIONS'], ['get', 'getter', 'retrieve']),
    ('Setter', ['FUNCTIONS'], ['set', 'setter', 'update', 'assign']),
    ('Iterator', ['FUNCTIONS'], ['iter', 'iterator', 'next', 'hasnext']),

    # Mais 40+ hadrons restantes...
    ('Constant', ['VARIABLES'], ['const', 'final', 'readonly', 'define']),
    ...
    ('Comment', ['STATEMENTS'], ['comment', 'note', 'doc']),
    ('Annotation', ['STATEMENTS'], ['@', 'annotation', 'metadata']),

    ('StringLiteral', ['PRIMITIVES'], ['string', 'text', 'char']),
    ('NumericLiteral', ['PRIMITIVES'], ['number', 'int', 'float', 'decimal']),
    ('BooleanLiteral', ['PRIMITIVES'], ['boolean', 'bool', 'true', 'false']),
    ('NullLiteral', ['PRIMITIVES'], ['null', 'undefined', 'nil', 'none']),

    ('ArrayLiteral', ['PRIMITIVES'], ['array', 'list', 'vector', 'collection']),
    ('ObjectLiteral', ['PRIMITIVES'], ['object', 'map', 'dict', 'hash']),

    ('ImportStatement', ['FILES'], ['import', 'require', 'include']),
    ('ExportStatement', ['MODULES'], ['export', 'module.exports']),
...
LANGUAGE_PATTERNS = {
    ...
    'javascript': {
        'extension': ['.js', '.jsx'],
        'patterns': {
            'function': r'^\s*(?:async\s+)?function\s+(\w+)|^(?:const|let|var)\s+(\w+)\s*=.*\(',
            'class': r'^\s*class\s+(\w+)',
            'import': r'^\s*import.*from\s+[\'"]|^.*require\(',
            'export': r'^\s*export\s+.*?(?:class|function|const|let|var)\s+(\w+)',
    ...
    'java': {
        'extension': ['.java'],
        'patterns': {
            'function': r'^\s*(?:public|private|protected)?\s*(?:static)?\s*(?:final)?\s*\w*\s*(\w+)\s*\(',
            'class': r'^\s*(?:public\s+)?class\s+(\w+)|interface\s+(\w+)',
    ...
    'csharp': {
        'extension': ['.cs'],
        'patterns': {
            'function': r'^\s*(?:public|private|protected)?\s*(?:static)?\s*(?:async)?\s*\w*\s*(\w+)\s*\(',
            'class': r'^\s*(?:public|internal)?\s*(?:abstract\s+)?class\s+(\w+)|interface\s+(\w+)',
            'using': r'^\s*using\s+([\w.]+);',
    ...
    'kotlin': {
        'extension': ['.kt'],
        'patterns': {
            'function': r'^\s*(?:fun\s+)(\w+)\s*\(',
            'class': r'^\s*(?:class|interface|object)\s+(\w+)',
            'import': r'^\s*import\s+([\w.]+)',
            'variable': r'^\s*(?:var|val)\s+(\w+)',
            'comment': r'^\s*//.*'
        }
    }
}
...
class SpectrometerV6:
    ...
    def extract_elements(self, file_path: Path, language: str) -> List[Dict[str, Any]]:
        """Extrai elementos usando regex patterns"""
        elements = []
    ...
    def _detect_quarks(self, line: str, element_type: str) -> List[str]:
        ...
        return detected  # Retorna lista vazia se não detectar nada
    ...
    def _classify_hadrons(self, line: str, name: str, element_type: str, quarks: List[str]) -> List[str]:
        ...
        if element_type == 'function':
            ...
            elif name_lower in ['__init__', 'constructor']:
            ...
            elif any(p in name_lower for p in ['model', 'entity']):
    ...
    def analyze_file(self, file_path: Path) -> List[Dict[str, Any]]:
        ...
        return self.extract_elements(file_path, language)
...
"""
        ...
        report += "\n🎯 TOP 30 HÁDRONS:\n"
        top_hadrons = sorted(self.stats['hadrons_detected'].items(), key=lambda x: x[1], reverse=True)[:30]
        for hadron, count in top_hadrons:
            if count > 0:
                report += f"  • {hadron:25} {count:4} ocorrências\n"

        # Score
        score = min(100, (len(self.stats['hadrons_detected']) * 3))
...
def demo_spectrometer_v6():
    ...
    print("🚀 SPECTROMETER v6 - FINAL VERSION")
    print("=" * 60)
